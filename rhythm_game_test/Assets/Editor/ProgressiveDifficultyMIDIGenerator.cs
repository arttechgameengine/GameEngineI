...
public class ProgressiveDifficultyMIDIGenerator : EditorWindow
{
    ...
    private float noteSpeed = 500f;  // 노트 이동 속도
    ...
    [MenuItem("Tools/Progressive Difficulty MIDI Generator")]
    public static void ShowWindow()
    {
        GetWindow<ProgressiveDifficultyMIDIGenerator>("Progressive MIDI Generator");
    }

    private void OnGUI()
    {
        GUILayout.Label("Progressive Difficulty MIDI Generator", EditorStyles.boldLabel);
        ...
        if (useProgressiveDifficulty)
        {
            ...
            EditorGUILayout.HelpBox(
                "노트 속도: 속도가 느릴수록 노트 개수가 자동으로 줄어듭니다.\n" +
                "기준 속도(500): 지정한 개수 그대로 / 1000: 그대로 유지 / 250: 약 50% 감소",
                MessageType.Info
            );
        ...
        if (GUILayout.Button("Generate Progressive Chart", GUILayout.Height(40)))
        {
            if (midiFile != null)
            {
                ConvertMidiToJson();
    ...
    private void ConvertMidiToJson()
    {
        ...
        try
        {
            ...
            string fileName = $"{chart.songName}_progressive_chart.json";
            ...
            EditorUtility.DisplayDialog(
                "Success",
                $"Progressive chart generated!\n\nNotes: {chartNotes.Count}\nSaved to: {fullPath}",
                "OK"
            );

            Debug.Log($"Progressive chart conversion complete: {chartNotes.Count} notes generated");
        }
