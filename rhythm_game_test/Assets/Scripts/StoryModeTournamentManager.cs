...
/// <summary>
/// Story Mode 전용 토너먼트 맵 관리
/// 진행도에 따라 라운드 잠금/해제 처리
/// </summary>
public class StoryModeTournamentManager : MonoBehaviour
{
    [Header("Cinemachine")]
    ...
    public TextMeshProUGUI progressText; // 진행도 표시 (예: "진행도: 2/5")
    ...
    void Start()
    {
        ...
        if (infoPanelBackground != null)
        {
            ...
            // Story Mode: 진행 상황에 따라 시작 라운드 결정
    currentRoundIndex = GetIntroStartRound();
    
    // 캐릭터를 시작 라운드 위치로 초기 배치
    if (playerCharacter != null && roundPositions[currentRoundIndex] != null)
    {
        playerCharacter.position = roundPositions[currentRoundIndex].position;
    }
    ...
    void SetupSilhouetteCanvasGroup()
    {
        if (tournamentSilhouettesContainer != null)
        {
            ...
            if (silhouetteCanvasGroup == null)
            {
                ...
                Debug.LogWarning("[Story Tournament] tournamentSilhouettesContainer에 CanvasGroup 컴포넌트를 미리 추가해주세요!");
                return;
            }
    ...
    void SetupScreenOverlayCanvasGroup()
    {
        if (screenOverlayUICanvas != null)
        {
            ...
            if (screenOverlayCanvasGroup == null)
            {
                ...
                Debug.Log("[Story Tournament] Screen Overlay Canvas에 CanvasGroup 자동 추가됨");
            }
    ...
    System.Collections.IEnumerator PlayIntroAnimation()
    {
        ...
        Debug.Log("[Story Tournament] 인트로 시작 - 전체 브래킷 표시");

        // 2단계: 잠시 대기 (전체 브래킷을 보여줌)
        yield return new WaitForSeconds(2f);

        // 3단계: Round 1 카메라를 먼저 target position으로 설정
         Debug.Log($"[Story Tournament] Round {currentRoundIndex + 1} 카메라를 target position으로 설정");
    if (roundCameraTargetPositions[currentRoundIndex] != null)
    {
        roundCameras[currentRoundIndex].Follow = roundCameraTargetPositions[currentRoundIndex];
    }

        // 4단계: Round 1로 줌인 시작
        Debug.Log("[Story Tournament] 줌인 시작 - Round 1로");
        ...
        // 6단계: 실루엣 페이드아웃
        Debug.Log("[Story Tournament] 실루엣 페이드아웃 시작");
        ...
        // 7단계: 줌인 완료 후 UI 표시
        Debug.Log("[Story Tournament] 줌인 완료 - UI 표시 시작");
        ...
        Debug.Log("[Story Tournament] 인트로 완료");
    ...
    void SetupRoundCameraConfiners()
    {
        for (int i = 0; i < roundCameras.Length; i++)
        {
            if (roundCameras[i] != null && roundBoundaries[i] != null)
            {
                ...
                {
                    // AddComponent 대신 에디터에서 미리 추가하도록 경고
                    Debug.LogWarning($"[Story Tournament] Round {i + 1} 카메라에 CinemachineConfiner2D 컴포넌트를 미리 추가해주세요!");
                    continue;
                }
                confiner.BoundingShape2D = roundBoundaries[i];
                Debug.Log($"[Story Tournament] Round {i + 1} 카메라 Confiner 설정 완료");
            }
        }
    }
    ...
    void SetupCameraFollowTarget()
    {
        ...
        {
            Debug.LogWarning("[Story Tournament] 플레이어 캐릭터가 설정되지 않았습니다.");
            return;
        }
        ...
        for (int i = 0; i < roundCameras.Length; i++)
        {
            if (roundCameras[i] != null)
            {
                ...
                Debug.Log($"[Story Tournament] Round {i + 1} 카메라 Follow → playerCharacter 설정 완료");
            }
        }
    }
    ...
    void ShowRound(int roundIndex)
    {
        ...
        {
            Debug.LogWarning($"[Story Tournament] 잘못된 라운드 인덱스: {roundIndex}");
            return;
        }

        // Story Mode: 잠긴 라운드는 화살표로 이동 불가
        if (GameModeManager.Instance.IsRoundLocked(roundIndex))
        {
            Debug.Log($"[Story Tournament] Round {roundIndex + 1}는 잠겨있습니다!");
            return;
        }

        if (isCharacterMoving)
        {
            Debug.Log("[Story Tournament] 캐릭터 이동 중에는 조작할 수 없습니다.");
            return;
        }
    ...
    System.Collections.IEnumerator MoveToRound(int targetRoundIndex)
    {
        ...
        {
            Debug.LogError($"[Story Tournament] 캐릭터 또는 Round {targetRoundIndex + 1} 위치가 설정되지 않았습니다!");
        ...
        Debug.Log($"[Story Tournament] Round {targetRoundIndex + 1} 이동 완료");
    }
    ...
    {
        Debug.Log("[Story Tournament] 주인공 + Screen Overlay UI Fade In 시작");
        ...
        Debug.Log("[Story Tournament] 주인공 + Screen Overlay UI Fade In 완료");
    }
    ...
    System.Collections.IEnumerator RepositionCameraToTarget(int roundIndex)
    {
        ...
        {
            Debug.LogWarning($"[Story Tournament] Round {roundIndex + 1} 카메라 목표 위치 인덱스가 범위를 벗어났습니다.");
            yield break;
        }

        if (roundCameraTargetPositions[roundIndex] == null)
        {
            Debug.LogWarning($"[Story Tournament] Round {roundIndex + 1} 카메라 목표 위치가 설정되지 않았습니다. 재배치를 건너뜁니다.");
            yield break;
        }
        ...
        {
            Debug.LogWarning($"[Story Tournament] Round {roundIndex + 1} 카메라가 설정되지 않았습니다.");
            yield break;
        }
        ...
        Debug.Log($"[Story Tournament] Round {roundIndex + 1} 카메라를 목표 위치로 재배치 중... ({cameraRepositionDuration}초)");

        // 카메라가 목표 위치로 이동할 시간 대기
        yield return new WaitForSeconds(cameraRepositionDuration);

        Debug.Log($"[Story Tournament] Round {roundIndex + 1} 카메라 재배치 완료");
    }
    ...
    void OnPreviousRound()
    {
        ...
        if (prevIndex < 0)
        {
            Debug.Log("[Story Tournament] 첫 번째 라운드입니다.");
            return;
        }

        // Story Mode: 이전 라운드도 잠금 확인 (혹시 모를 경우)
        if (GameModeManager.Instance.IsRoundLocked(prevIndex))
        {
            Debug.Log($"[Story Tournament] Round {prevIndex + 1}는 잠겨있습니다!");
            return;
        }
    ...
    void OnNextRound()
    {
        ...
        if (nextIndex >= 5)
        {
            Debug.Log("[Story Tournament] 마지막 라운드입니다.");
            return;
        }

        // Story Mode: 다음 라운드 잠금 확인
        if (GameModeManager.Instance.IsRoundLocked(nextIndex))
        {
            Debug.Log($"[Story Tournament] Round {nextIndex + 1}는 잠겨있습니다!");
            return;
        }
    ...
    void UpdateNavigationButtons()
    {
        ...
        {
            // 이전 라운드가 있고 잠겨있지 않으면 활성화
            bool canGoPrevious = currentRoundIndex > 0 && !GameModeManager.Instance.IsRoundLocked(currentRoundIndex - 1);
            leftArrowButton.interactable = canGoPrevious;
        }

        if (rightArrowButton != null)
        {
            // Story Mode: 다음 라운드가 해금되어 있고 범위 내에 있으면 활성화
            bool nextRoundUnlocked = (currentRoundIndex < 4) && !GameModeManager.Instance.IsRoundLocked(currentRoundIndex + 1);
            rightArrowButton.interactable = nextRoundUnlocked;
        }
    }

    /// <summary>
    /// Story Mode 진행도 텍스트 업데이트
    /// </summary>
    void UpdateProgressText()
    {
        if (progressText != null)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                if (GameModeManager.Instance.roundCleared[i])
                    clearedCount++;
            }

            progressText.text = $"진행도: {clearedCount}/5";
            Debug.Log($"[Story Tournament] 진행도: {clearedCount}/5");
        }
    }
    ...
    {
        if (roundIndex < 0 || roundIndex >= GameModeManager.Instance.allRounds.Length)
        {
            Debug.LogError($"[Story Tournament] 잘못된 라운드 인덱스: {roundIndex}");
            return;
        }
        ...
        if (enemyPortraitImage != null && round.enemyPortrait != null)
        {
        ...
        Debug.Log($"[Story Tournament] Round {roundIndex + 1} 정보 패널 열림");
    }
    ...
    void CloseInfoPanel()
    {
        ...
        Debug.Log("[Story Tournament] 라운드 정보 패널 닫힘");
    }
    ...
    {
        // Story Mode: 잠긴 라운드는 시작 불가
        if (GameModeManager.Instance.IsRoundLocked(currentRoundIndex))
        {
            Debug.Log($"[Story Tournament] Round {currentRoundIndex + 1}는 잠겨있습니다!");
            return;
        }

        Debug.Log($"[Story Tournament] Round {currentRoundIndex + 1} 대화씬으로 이동!");

        // PlayerPrefs에 현재 라운드 저장
    ...
    Color GetDifficultyColor(RoundData.Difficulty difficulty)
    {
        switch (difficulty)
        {
            case RoundData.Difficulty.Easy: return Color.green;
            case RoundData.Difficulty.Normal: return Color.yellow;
            case RoundData.Difficulty.Hard: return new Color(1f, 0.5f, 0f); // 주황색
            case RoundData.Difficulty.VeryHard: return Color.red;
    ...
    void ShowButtons(bool show)
    {
        ...
        Debug.Log($"[Story Tournament] 버튼 {(show ? "표시" : "숨김")}");
    }

    /// <summary>
    /// Story Mode: 라운드 클리어 후 호출 (ScoreResultUI에서 호출)
    /// </summary>
    public void OnRoundCleared(int roundIndex)
    {
        Debug.Log($"[Story Tournament] Round {roundIndex + 1} 클리어!");

        // GameModeManager에 클리어 정보 저장
        GameModeManager.Instance.ClearRound(roundIndex);

        // 라운드 버튼 상태 업데이트 (클리어 표시)
        if (roundIndex >= 0 && roundIndex < roundButtons.Length)
        {
            roundButtons[roundIndex].Setup(roundIndex, GameModeManager.Instance.allRounds[roundIndex]);
        }

        // 다음 라운드 해금 (이미 GameModeManager.ClearRound에서 처리됨)
        if (roundIndex + 1 < roundButtons.Length)
        {
            bool isLocked = GameModeManager.Instance.IsRoundLocked(roundIndex + 1);
            roundButtons[roundIndex + 1].SetLocked(isLocked);
        }
        ...
        // 클리어된 라운드 보여주고 → 다음 라운드로 자동 이동
        StartCoroutine(ShowClearedThenMoveNext(roundIndex));
    }

    /// <summary>
    /// 클리어된 라운드 잠시 보여주고 다음 라운드로 자동 이동
    /// </summary>
    System.Collections.IEnumerator ShowClearedThenMoveNext(int clearedRoundIndex)
    {
        // 클리어된 라운드로 이동
        currentRoundIndex = clearedRoundIndex;
        if (currentRoundIndicator != null)
        {
            currentRoundIndicator.text = $"Round {clearedRoundIndex + 1} / 5";
        }
        ...
        // 다음 라운드가 있으면 자동 이동
        if (clearedRoundIndex + 1 < 5)
        {
            Debug.Log($"[Story Tournament] 다음 라운드로 자동 이동: Round {clearedRoundIndex + 2}");
            ShowRound(clearedRoundIndex + 1);
        }
        else
        {
            Debug.Log("[Story Tournament] 모든 라운드 클리어!");
        }
    }

    // ===============================================
// ★ 현재 진행 상태에서 "인트로 후 처음 보여줄 라운드" 계산
// ===============================================
int GetIntroStartRound()
{
    int lastCleared = -1;

    // RoundCleared 배열을 기반으로 마지막 클리어 라운드 찾기
    for (int i = 0; i < 5; i++)
    {
        if (GameModeManager.Instance.roundCleared[i])
            lastCleared = i;
    ...
    // 마지막 클리어가 Round4였다면(최종스테이지) → Round5는 없음, 그대로 4
    if (lastCleared == 4)
        return 4;
